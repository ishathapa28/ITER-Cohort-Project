...
Solve LeetCode problem **146: LRU Cache** (Medium). 
...
**Hash Map + Doubly Linked List (O(1) Access & Eviction)**
...
The optimal approach utilizes **Hash Map + Doubly Linked List (O(1) Access & Eviction)** to trade minimal memory space for immediate $\mathcal{O}(1)$ average lookups, eliminating redundant scans.
