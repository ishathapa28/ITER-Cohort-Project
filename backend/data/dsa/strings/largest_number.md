...
Solve standard LeetCode problem **179: Largest Number** (Medium).

## Pattern

Custom Sorting Comparator (`a+b` vs `b+a`)
...
Apply the **Custom Sorting Comparator (`a+b` vs `b+a`)** strategy to prune invalid branches, avoid redundant character operations, or process string indices efficiently.
